...
import type { CurrencyCode, ForecastHorizon, ForecastMode, ForecastResult, MonthEntry } from "@/lib/types";
...
export default function HomePage() {
  ...
  const [horizon, setHorizon] = React.useState<ForecastHorizon>(3);
  ...
  const handleGenerate = () => {
    ...
    window.setTimeout(() => {
      try {
        const forecast = generateForecast(entries, mode, horizon);
        setResult(forecast);
        toast({
          title: "Forecast generated",
          description: `Projected ${mode === "profit" ? "profit" : "expenses"} for the next ${horizon} months are ready.`,
        });
