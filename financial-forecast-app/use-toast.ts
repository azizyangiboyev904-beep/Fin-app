...
export function ForecastTable({ entries, predictions, currency }: ForecastTableProps) {
  ...
  return (
    <Card id="forecast-table">
      <CardHeader>
        <CardTitle className="text-xl">{entries.length + predictions.length}-month view</CardTitle>
        <CardDescription>
          Your {entries.length} actual months followed by the {predictions.length}-month projection.
        </CardDescription>
      </CardHeader>
      <CardContent>
