import type { ForecastHorizon, ForecastMode, ForecastResult, MonthEntry, PredictedMonth } from "./types";
 ...
 */
function getNextMonthNames(entries: MonthEntry[], horizon: number): string[] {
  const MONTHS = [
  ...
  const offsets = Array.from({ length: horizon }, (_, i) => i + 1);

  if (idx === -1) {
    return offsets.map((offset) => `Month ${entries.length + offset}`);
  }

  // Roll past December into the following year — append the year once we
  // wrap, so a 12-month forecast starting in October reads Nov, Dec, Jan '27...
  const startYear = new Date().getFullYear();
  return offsets.map((offset) => {
    const monthIdx = (idx + offset) % 12;
    const yearsForward = Math.floor((idx + offset) / 12);
    return yearsForward > 0 ? `${MONTHS[monthIdx]} '${String(startYear + yearsForward).slice(-2)}` : MONTHS[monthIdx];
  });
}
...
export function generateForecast(
  ...
  const nextMonthNames = getNextMonthNames(entries, horizon);
  ...
  const predictions: PredictedMonth[] = nextMonthNames.map((month, i) => {
    ...
    // Momentum decays the further out we project — a growth rate observed
    // over 3 months is a reasonable guide for next month, but compounding it
    // unchecked across a 12-month horizon would produce runaway numbers.
    // The regression line (linear, not compounding) takes over more as i grows.
    const momentumDecay = Math.pow(0.88, i);
    const stepTrendWeight = trendWeight * momentumDecay;
    const stepRegressionWeight = 1 - stepTrendWeight;

    const blended = regressionValue * stepRegressionWeight + compoundedValue * stepTrendWeight;
